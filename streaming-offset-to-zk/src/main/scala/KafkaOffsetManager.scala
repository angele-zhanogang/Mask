   ...
   def readOffsets(zkClient: ZkClient, zkOffsetPath: String, topic: String): Option[Map[TopicAndPartition, Long]] = {
    ...
    offsetsRangesStrOpt match {
      case Some(offsetsRangesStr) =>
        //这个topic在zk里面最新的分区数量
        val  lastest_partitions: Seq[Int] = ZkUtils.getPartitionsForTopics(zkClient,Seq(topic)).get(topic).get
        val z1: mutable.Map[String, Seq[Int]] = ZkUtils.getPartitionsForTopics(zkClient,Seq(topic))
        val z2: Option[Seq[Int]] = z1.get(topic)
        val z3: Seq[Int] = z2.get
