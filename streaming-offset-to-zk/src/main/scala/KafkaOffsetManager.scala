...
object KafkaOffsetManager {
   ...
   def readOffsets(zkClient: ZkClient, zkOffsetPath: String, topic: String): Option[Map[TopicAndPartition, Long]] = {
    ...
    offsetsRangesStrOpt match {
      case Some(offsetsRangesStr) =>
        //这个topic在zk里面最新的分区数量
        val  lastest_partitions: Seq[Int] = ZkUtils.getPartitionsForTopics(zkClient, Seq(topic))(topic)
        ...
        if(offsets.size<lastest_partitions.size){
          //得到旧的所有分区序号
          val old_partitions: Array[Int] =offsets.keys.map(p=>p.partition).toArray
          //通过做差集得出来多的分区数量数组
          val add_partitions: Seq[Int] =lastest_partitions.diff(old_partitions)
  ...
  def saveOffsets(zkClient: ZkClient, zkOffsetPath: String, rdd: RDD[_]): Unit = {
    ...
    val offsetsRangesStr: String = offsetsRanges.map(offsetRange => s"${offsetRange.partition}:${offsetRange.untilOffset}").mkString(",")
